...
def _extract_meta_blob(text: str, filename: str) -> Dict[str, Any]:
    out: Dict[str, Any] = {
        ...
        "protective_equipment": None, "work_safe_guidance": None
    }
    ...
    # Extract Catalog/Product Number
    ...
    # Extract Protective and Work Safe Sections
    protective_pat = re.compile(
        r"(?i)\b(Protective\s+clothing|Personal\s+protective\s+equipment|PPE)\b\s*[:\-]?\s*(.+)")
    work_safe_pat = re.compile(
        r"(?i)\b(Handling\s+and\s+storage|Safe\s+handling|Storage\s+precautions)\b\s*[:\-]?\s*(.+)")

    for line in text.splitlines():
        m = protective_pat.search(line)
        if m and not out["protective_equipment"]:
            out["protective_equipment"] = m.group(2).strip()

        m = work_safe_pat.search(line)
        if m and not out["work_safe_guidance"]:
            out["work_safe_guidance"] = m.group(2).strip()
...
def ingest_file(
    ...
    # Heuristically extract metadata from the document
    ...
    for ch in sect_chunks:
        raw_meta = {
            ...
            "page": int(ch["page"]),
            "source_path": path,
            # Use user-provided manufacturer, fallback to extracted one
            "manufacturer": manufacturer or meta0.get("manufacturer"),
            # Use user-provided date, fallback to extracted one
            "revision_date": revision_date or meta0.get("revision_date"),
            "cas": meta0.get("cas"),
            # Add protective and work safe metadata
            "protective_equipment": meta0.get("protective_equipment"),
            "work_safe_guidance": meta0.get("work_safe_guidance"),
        }
