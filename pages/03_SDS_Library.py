...
def _load_all_docs() -> Tuple[List[Dict[str, Any]], bool]:
    """Pull metadatas for all docs from Chroma. This function is NOT cached."""
    vdb = get_vectorstore()
    docs: List[Any] = []
    used_private_get = False
    try:
        raw = vdb._collection.get(include=["metadatas", "documents"], where=None)
...
def _aggregate_materials(docs: List[Dict[str, Any]]) -> pd.DataFrame:
    """Aggregates metadata for a cleaner table view."""
...
def _display_material_details(material: str, df: pd.DataFrame, docs: List[Dict[str, Any]]):
    ...
    with col_aliases:
        ...
        with st.form(key="alias_editor"):
            ...
            if submit_button:
                ...
                st.success("Aliases saved successfully! Reloading page...")
                st.rerun()  # Use rerun to reload the new aliases
...
_display_material_details(selected_material, df, docs)